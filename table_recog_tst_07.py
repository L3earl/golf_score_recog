#!/usr/bin/env python3
"""
테이블 감지 및 크롭 테스트 스크립트 (Microsoft Donut 모델용)

이 스크립트는 data/raw_img 폴더의 이미지들을 가져와서
Microsoft의 Donut (Document Understanding Transformer) 모델을 사용하여
문서 구조 분석을 통해 테이블의 좌표를 감지하고, 해당 영역을 크롭하여 test 폴더에 저장합니다.

Microsoft Donut 모델은 문서 이해에 특화된 모델로,
...
def load_model():
    """Microsoft Donut 모델 로드"""
    try:
        from transformers import DonutProcessor, VisionEncoderDecoderModel
        
        logger.info("Microsoft Donut 모델을 로드하는 중...")
        ...
        logger.info("✅ Microsoft Donut 모델 로드 완료")
        logger.info("문서 이해 및 구조 분석 모델")
        return processor, model
        
    except Exception as e:
        logger.error(f"Microsoft Donut 모델 로드 실패: {e}")
        raise RuntimeError(f"Microsoft Donut 모델 로드에 실패했습니다: {e}")

def detect_tables(processor, model, image: Image.Image, threshold: float = 0.3) -> List[Tuple[float, float, float, float]]:
    """이미지에서 테이블 감지 (Microsoft Donut 모델용)"""
    try:
        # Donut 모델 사용
...
def detect_tables_donut(processor, model, image: Image.Image, threshold: float = 0.3) -> List[Tuple[float, float, float, float]]:
    """Microsoft Donut 모델을 사용한 테이블 감지"""
    try:
        ...
        print(f"\n{'='*60}")
        print(f"MICROSOFT DONUT 모델 출력 (sequence):")
        print(f"{'='*60}")
        ...
        logger.info(f"Microsoft Donut 모델 출력: {sequence}")
        ...
        if "table" in sequence.lower() or "score" in sequence.lower():
        ...
        logger.error(f"Microsoft Donut 테이블 감지 중 오류 발생: {e}")
        return []
...
def process_images(input_dir: str, output_dir: str, invert_colors: bool = False, threshold: float = 0.3) -> None:
    """모든 이미지 파일 처리 (Microsoft Donut 모델용)"""
    # 모델 로드
...
def main():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Microsoft Donut 모델을 사용한 테이블 감지 및 크롭 시작")
        logger.info("=" * 60)
