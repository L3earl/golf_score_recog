#!/usr/bin/env python3
"""
테이블 감지 및 크롭 테스트 스크립트

이 스크립트는 data/raw_img 폴더의 이미지들을 가져와서
Microsoft의 table-transformer-detection 모델을 사용하여
...
def load_model():
    """Microsoft table-transformer-detection 모델 로드"""
    try:
        from transformers import DetrImageProcessor, DetrForObjectDetection
        
        logger.info("Microsoft table-transformer-detection 모델을 로드하는 중...")
        processor = DetrImageProcessor.from_pretrained('microsoft/table-transformer-detection')
        model = DetrForObjectDetection.from_pretrained('microsoft/table-transformer-detection')
        
        logger.info("✅ Microsoft table-transformer-detection 모델 로드 완료")
        logger.info(f"모델 라벨 정보: {list(model.config.id2label.values())}")
        return processor, model
        
    except Exception as e:
        logger.error(f"Microsoft table-transformer-detection 모델 로드 실패: {e}")
        raise RuntimeError(f"Microsoft table-transformer-detection 모델 로드에 실패했습니다: {e}")
...
def detect_tables(processor, model, image: Image.Image, threshold: float = 0.3) -> List[Tuple[float, float, float, float]]:
    ...
    try:
        ...
        for score, label, box in zip(results['scores'], results['labels'], results['boxes']):
            ...
            # ❗️❗️ 핵심 수정: 'table' 라벨만 필터링 ❗️❗️
            if label_name == 'table' and score > threshold:
