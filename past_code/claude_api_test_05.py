"""
Claude API 테스트 스크립트 v05

의도: tst_img 폴더의 이미지들을 OpenCV DNN Super Resolution으로 업스케일링한 후 개선된 Claude API로 테스트
- OpenCV DNN Super Resolution 모델을 사용하여 이미지를 4배 업스케일링하여 선명하게 만듦
...
def upscale_image(image_path):
    """이미지를 OpenCV DNN Super Resolution으로 업스케일링하고 tst_img2 폴더에 저장
...
def main():
    ...
    의도: tst_img 폴더의 모든 이미지를 OpenCV DNN Super Resolution으로 업스케일링한 후 Claude API로 처리
    """
    print("=" * 60)
    print("Claude API 테스트 스크립트 v05 (OpenCV DNN Super Resolution)")
    print("=" * 60)
    ...
    try:
        # OpenCV DNN Super Resolution 모델 로드
        print("OpenCV DNN Super Resolution 모델 로딩 중...")
        load_superres_model()
