...
def calculate_crop_dimensions(first_hole, second_hole, t_char):
    ...
    if second_hole:
        ...
        second_hole_y = (second_hole_bbox[0][1] + second_hole_bbox[2][1]) / 2  # 두번째 HOLE의 Y는 중앙값 사용
...
def crop_golf_score_area(image_path, holes, valid_pairs, output_dir, target_width=None):
    ...
    try:
        ...
        # 첫번째 HOLE의 오른쪽 끝 좌표에서 시작
        hole_bbox = first_hole['bbox']
        start_x = int(hole_bbox[2][0])  # HOLE의 오른쪽 끝 x
        start_y = int(hole_bbox[0][1])  # HOLE의 좌상단 y
